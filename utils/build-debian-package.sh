...
PACKAGE_VERSION=$($SRC_PATH/src/maxeterd --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/maxeterd $DEB_BIN
cp $SRC_PATH/src/maxeter-cli $DEB_BIN
cp $SRC_PATH/util/fetch-params.sh $DEB_BIN/maxeter-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/maxeterd.1 $DEB_MAN
cp $SRC_DOC/man/maxeter-cli.1 $DEB_MAN
cp $SRC_DOC/man/maxeter-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/maxeterd.bash-completion $DEB_CMP/maxeterd
cp $SRC_PATH/contrib/maxeter-cli.bash-completion $DEB_CMP/maxeter-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/maxeterd.1
gzip --best -n $DEB_MAN/maxeter-cli.1
gzip --best -n $DEB_MAN/maxeter-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/maxeterd $DEB_BIN/maxeter-cli
