...
if [ "$uname_S" = "Darwin" ]; then
    PARAMS_DIR="$HOME/Library/Application Support/MaxeterParams"
else
    PARAMS_DIR="$HOME/.maxeter-params"
fi
...
DOWNLOAD_URL="https://github.com/maxeteristy/maxeter/releases/tag/1.2.15.3"
...
# fetch methods can be disabled with MAXE_DISABLE_SOMETHING=1
MAXE_DISABLE_WGET="${MAXE_DISABLE_WGET:-}"
#MAXE_DISABLE_IPFS="${MAXE_DISABLE_IPFS:-}"
MAXE_DISABLE_CURL="${MAXE_DISABLE_CURL:-}"

LOCKFILE=/tmp/fetch_params.lock

fetch_wget() {
    if [ -z "$WGETCMD" ] || ! [ -z "$MAXE_DISABLE_WGET" ]; then
        return 1
    fi
...
#fetch_ipfs() {
#    if [ -z "$IPFSCMD" ] || ! [ -z "$MAXE_DISABLE_IPFS" ]; then
#        return 1
#    fi
...
fetch_curl() {
    if [ -z "$CURLCMD" ] || ! [ -z "$MAXE_DISABLE_CURL" ]; then
        return 1
    fi
...
Failed to fetch the Maxeter parameters!
...
main() {
    ...
    cat <<EOF
MAXE - fetch-params.sh

This script will fetch the Maxeter SNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common Maxeter SNARK parameters. Note that it is
