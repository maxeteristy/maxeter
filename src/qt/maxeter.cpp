...
{
    return QCoreApplication::translate("maxeter-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in maxeter.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in maxeter.qrc)
...
/** Class encapsulating Maxeter Core startup and shutdown.
...
/** Main Maxeter application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // maxeter: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Maxeter Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-windowtitle=<name>", _("Sets a window title which is appended to \"Maxeter Core - \""), false, OptionsCategory::GUI);
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(maxeter);
    Q_INIT_RESOURCE(maxeter_locale);
    ...
    /// 6. Determine availability of data and blocks directory and parse maxeter.conf
    ...
    // maxeter: links repeatedly have their payment requests routed to this process:
