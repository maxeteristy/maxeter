...
const char * const BITCOIN_CONF_FILENAME = "maxeter.conf";
const char * const BITCOIN_PID_FILENAME = "maxeterd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "maxeterd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MaxeterCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MaxeterCore
    // Mac: ~/Library/Application Support/MaxeterCore
    // Unix: ~/.maxetercore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MaxeterCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MaxeterCore";
#else
    // Unix
    return pathRet / ".maxetercore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty maxeter.conf if it does not excist
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Maxeter Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Maxeter Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2021, nEndYear) + "The Maxeter Core developers";
    }
