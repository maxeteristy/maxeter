...
{
    const char* pszTimestamp = "The Maxeter have to correct the total chain work for the current difficulty 2023";
...
public:
    CMainParams() {
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Maxeter: 1 day
        consensus.nPowTargetSpacing = 3 * 60; // Maxeter: 3 minutes
        ...
        consensus.defaultAssumeValid = uint256S("0x00059d69753f177eed308c9f122e1c7cfb25097ee0ed3e948050b9acaaed23e7"); // block hash for 0
        ...
        genesis = CreateGenesisBlock(1698028116, 3803, 0x20001fff, 4, 5000 * COIN);
        VerifyGenesisPOW(genesis);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x7cc32c8e56edea8518e05ecc38df6152c85e1869bfae0eb785db898df67a1903"));
        assert(genesis.hashMerkleRoot == uint256S("0xb461e1dafd7e4fd9173c52654aa332b9ada87d1affc3a9f9214357ccab4d8acf"));

        vSeeds.emplace_back("node01.maxeter.com");
        vSeeds.emplace_back("node02.maxeter.com");        


        // Maxeter addresses start with 'M'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,50);
        // Maxeter script addresses start with '7'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,16);
        // Maxeter private keys start with '7' or 'X'
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,128);
        // Maxeter BIP32 pubkeys start with 'xpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x88, 0xB2, 0x1E};
        // Maxeter BIP32 prvkeys start with 'xprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x88, 0xAD, 0xE4};
        // Maxeter BIP44 coin type is '5'
...
//        }
        ...
        vSporkAddresses = {"MDBkxATwUjyf8h8NHKtuVUWgBp3XcbZo1n"};
        nMinSporkKeys = 1;
        ...
        chainTxData = ChainTxData{
            1698028116,   // * UNIX timestamp of last known number of transactions (Block 0)
...
public:
    CTestNetParams() {
        ...
        consensus.powLimit = uint256S("00ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"); // ~uint256(0) >> 20
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Maxeter: 1 day
        consensus.nPowTargetSpacing = 60; // Maxeter: 1 minutes
        ...
        vSeeds.emplace_back("0xt.maxeter.com");

        // Testnet Maxeter addresses start with 'r'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,123);
        // Testnet Maxeter script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Testnet private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Testnet Maxeter BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Testnet Maxeter BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Testnet Maxeter BIP44 coin type is '1' (All coin's testnet default)
        std::string strExtCoinType = gArgs.GetArg("-extcoinindex", "");
...
public:
    CDevNetParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 2; // DIP0003 activated immediately on devnet
        ...
        consensus.nPowTargetTimespan = 24 * 60 * 60; // Maxeter: 1 day
        consensus.nPowTargetSpacing = 2 * 60; // Maxeter: 2 minutes
        consensus.fPowAllowMinDifficultyBlocks = true;
        consensus.fPowNoRetargeting = false;
        consensus.nPowDGWHeight = 60;
        consensus.DGWBlocksAvg = 60;
...
//      std::cout << "hash: " << consensus.hashGenesisBlock.ToString() << std::endl;
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        //vSeeds.push_back(CDNSSeedData("maxeterevo.org",  "devnet-seed.maxeterevo.org"));

        // Testnet Maxeter addresses start with 'y'
...
public:
    CRegTestParams() {
       ...
       // consensus.DIP0003EnforcementHeight = 500;
        ...
        // Regtest Maxeter addresses start with 'y'
        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1,140);
        // Regtest Maxeter script addresses start with '8' or '9'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1,19);
        // Regtest private keys start with '9' or 'c' (Bitcoin defaults)
        base58Prefixes[SECRET_KEY] =     std::vector<unsigned char>(1,239);
        // Regtest Maxeter BIP32 pubkeys start with 'tpub' (Bitcoin defaults)
        base58Prefixes[EXT_PUBLIC_KEY] = {0x04, 0x35, 0x87, 0xCF};
        // Regtest Maxeter BIP32 prvkeys start with 'tprv' (Bitcoin defaults)
        base58Prefixes[EXT_SECRET_KEY] = {0x04, 0x35, 0x83, 0x94};

        // Regtest Maxeter BIP44 coin type is '1' (All coin's testnet default)
        nExtCoinType = 1;
