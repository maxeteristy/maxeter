...
#if defined(NDEBUG)
# error "Maxeter Core cannot be compiled without assertions."
#endif
                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Maxeter
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    //     exit(1);
    // }
    double nSubsidy = 1800;      // (declaring the reward variable and its original/default amount)
    ...
    if (nPrevHeight < 10) {
        nSubsidy = 100; 
    }else if (nPrevHeight < 490) {
        nSubsidy = 5000; 
    }else if (nPrevHeight < 970) {
        nSubsidy = 4000; 
    }else if (nPrevHeight < 1440) {
        nSubsidy = 3000; 
    }else if (nPrevHeight < 1920) {
        nSubsidy = 2000; 
...
static int64_t nTimeMaxeterSpecific = 0;
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    /// MAXETER: Check superblock start
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, view, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(MAXETER): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    // MAXETER : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if (llmq::RejectConflictingBlocks()) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(MAXETER): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else if (!fReindex && !fImporting) {
        LogPrintf("ConnectBlock(MAXETER): spork is off, skipping transaction locking checks\n");
    }
    ...
    // MAXETER : MODIFIED TO CHECK SMARTNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, (blockReward + specialTxFees), strError)) {
        return state.DoS(0, error("ConnectBlock(MAXETER): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }
    ...
    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward, specialTxFees)) {
        return state.DoS(0, error("ConnectBlock(MAXETER): couldn't find smartnode or superblock payments"),
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeMaxeterSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Maxeter specific: %.2fms [%.2fs (%.2fms/blk)]\n", MILLI * (nTime5 - nTime4), nTimeMaxeterSpecific * MICRO, nTimeMaxeterSpecific * MILLI / nBlocksTotal);
...
bool CChainState::RollforwardBlock(const CBlockIndex* pindex, CCoinsViewCache& inputs, const CChainParams& params)
{
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, inputs, false /*fJustCheck*/, false /*fScriptChecks*/)) {
        return error("RollforwardBlock(MAXE): ProcessSpecialTxsInBlock for block %s failed with %s",
            pindex->GetBlockHash().ToString(), FormatStateMessage(state));
    }
