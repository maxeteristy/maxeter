...
# maxeter_hash
RUN git clone https://github.com/maxeteristy/maxeter_hash
RUN cd maxeter_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} maxeter
RUN useradd -u ${USER_ID} -g maxeter -s /bin/bash -m -d /maxeter maxeter
...
RUN mkdir /maxeter-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /maxeter-src && \
